...
set[str] check4mp(leaf(), GGrammar g) = {n | str n <- g.nts, /nonterminal(_) !:= g.prods[n]};
bool     check4mp(leaf(), GGrammar g, str n) = n in g.nts && /nonterminal(_) !:= g.prods[n];
...
set[str] check4mp(disallowed(), GGrammar g) = {n | str n <- g.nts, [production(n,empty())] := g.prods[n]};
